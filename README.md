...
- **Bateria (cliente): geometria fixa pré-calculada.** A casca, a malha, o halo e os anéis da esfera (~6.000 quads, ~25.000 vértices por bateria por frame) são convertidos uma vez em `float[]` de posições e normais; um frame só copia números para o buffer, sem alocar nem calcular raiz quadrada por vértice. Além de 24 blocos o halo e os rastros de energia não são desenhados. Visual idêntico de perto.
- **Cabo de itens (cliente): uma passada por frame.** A posição de cada item em trânsito é calculada uma vez por frame e agrupada pelo cabo em que está; cada cabo transparente só consulta o seu grupo. Antes, todo cabo visível percorria todos os trajetos do mundo a cada frame (cabos × itens), criando `Vec3` por cálculo e um `ItemStackRenderState` novo por item por frame; o modelo do item agora é resolvido uma vez por trajeto.
