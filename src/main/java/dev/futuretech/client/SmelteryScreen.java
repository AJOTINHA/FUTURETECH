...
public final class SmelteryScreen extends AbstractContainerScreen<SmelteryMenu> implements TabbedScreen {
    ...
    /** The progress arrow sits in the gap between the second ingredient and the alloy, centred on the row. */
    ...
    /** The crucible sits under the ingredient pair, however many lanes there are. */
    private static final int CRUCIBLE_X = 49;
    private static final int CRUCIBLE_BELOW_ROW = 20;
    ...
    private static final int CRUCIBLE_IDLE = 0xFF8B959F;
    private static final int CRUCIBLE_WALL = 0xFF56616D;
    private static final int MELT = 0xFFF86206;
    private static final int MELT_HOT = 0xFFFEB708;
    ...
    public SmelteryScreen(SmelteryMenu menu, Inventory inventory, Component title) {
        super(menu, inventory, title, SmelteryMenu.IMAGE_WIDTH, 184 + menu.extraHeight());
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        // Each lane's melt runs from its ingredients towards the alloy slot.
        ...
        drawCrucible(graphics, x + CRUCIBLE_X, y + menu.rowY(menu.lanes() - 1) + CRUCIBLE_BELOW_ROW, menu.isWorking());
        SortToggle.draw(graphics, menu, x + SortToggle.X, y + sortY(), mouseX, mouseY);
        tabs.render(graphics, x, y, imageWidth, mouseX, mouseY);
    }

    /** A crucible in the furnace status indicator position: grey when cold, orange with a hot surface at work. */
    private static void drawCrucible(GuiGraphicsExtractor graphics, int x, int y, boolean working) {
        int wall = working ? CRUCIBLE_WALL : CRUCIBLE_IDLE;
        // Rim, then walls that step in towards the floor.
        graphics.fill(x, y, x + 14, y + 2, wall);
        graphics.fill(x + 1, y + 2, x + 3, y + 9, wall);
        graphics.fill(x + 11, y + 2, x + 13, y + 9, wall);
        graphics.fill(x + 2, y + 9, x + 12, y + 11, wall);
        graphics.fill(x + 3, y + 11, x + 11, y + 12, wall);
        if (!working) return;
        graphics.fill(x + 3, y + 3, x + 11, y + 9, MELT);
        graphics.fill(x + 3, y + 3, x + 11, y + 4, MELT_HOT);
        graphics.fill(x + 5, y + 6, x + 6, y + 7, MELT_HOT);
        graphics.fill(x + 8, y + 7, x + 9, y + 8, MELT_HOT);
    }
    ...
    private int energyTop() {
        int rowsMiddle = menu.rowY(0) + menu.lanes() * SmelteryMenu.ROW_SPACING / 2;
