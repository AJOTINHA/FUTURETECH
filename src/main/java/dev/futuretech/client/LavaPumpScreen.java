...
public final class LavaPumpScreen extends AbstractContainerScreen<LavaPumpMenu> implements TabbedScreen {
    ...
    private static final int TANK_Y = LavaPumpMenu.INPUT_Y;
    private static final int TANK_WIDTH = 16;
    private static final int TANK_HEIGHT = LavaPumpMenu.OUTPUT_Y + 18 - LavaPumpMenu.INPUT_Y;
    ...
    public LavaPumpScreen(LavaPumpMenu menu, Inventory inventory, Component title) {
        super(menu, inventory, title, LavaPumpMenu.IMAGE_WIDTH, 184);
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        drawDownArrow(graphics, x + LavaPumpMenu.SLOT_X + 5, y + LavaPumpMenu.INPUT_Y + 21);
    ...
    private void drawTank(GuiGraphicsExtractor graphics, int x, int y) {
        ...
        int height = Math.round(lavaBar.width(menu.lavaStored(), menu.tankCapacity(), TANK_HEIGHT, menu.isSynced()));
        if (height > 0) {
    ...
    protected void extractTooltip(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        if (mouseX >= leftPos + TANK_X - 1 && mouseX < leftPos + TANK_X + TANK_WIDTH + 1
            ...
            // The tank, and how much of the pool is still waiting under the pipe.
            graphics.setTooltipForNextFrame(java.util.List.of(
                    Component.translatable("gui.futuretech.tank.contents",
                            Fluids.LAVA.getFluidType().getDescription(), menu.lavaStored(), menu.tankCapacity()).getVisualOrderText(),
                    Component.translatable("gui.futuretech.lava_pump.pool", menu.poolSize()).getVisualOrderText()), mouseX, mouseY);
        }
    ...
    protected void extractLabels(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        graphics.text(font, Component.translatable("gui.futuretech.tank.amount", menu.lavaStored(),
                menu.tankCapacity()), 70, 73, TEXT, false);
        // What the pump makes a tick, on its own line under the tank's level.
        graphics.text(font, Component.translatable("gui.futuretech.lava_pump.flow", menu.lavaRatePerTick()), 70, 83, TEXT, false);
        var status = menu.status();
        graphics.text(font, Component.translatable(status.key()), 70, 36, status.isProblem() ? 0xFFB03A2E : TEXT, false);
    }
}
