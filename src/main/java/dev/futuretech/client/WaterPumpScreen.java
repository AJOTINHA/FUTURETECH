...
import static dev.futuretech.block.entity.WaterPumpBlockEntity.MIN_SOURCES;
...
public final class WaterPumpScreen extends AbstractContainerScreen<WaterPumpMenu> {
    ...
    private static final int TANK_HEIGHT = WaterPumpMenu.OUTPUT_Y + 18 - WaterPumpMenu.INPUT_Y;
    ...
    public WaterPumpScreen(WaterPumpMenu menu, Inventory inventory, Component title) {
        ...
        // One line lower than the other machines: the flow line takes the row above the label.
        inventoryLabelY = 92;
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        drawDownArrow(graphics, x + WaterPumpMenu.SLOT_X + 5, y + WaterPumpMenu.INPUT_Y + 21);
        drawTank(graphics, x + TANK_X, y + TANK_Y);
        // The energy box is centred on the input slot's vertical span, as on the solid fuel pump.
    ...
    /** The water column, drawn with water's own texture and tint like the fluid tank's bar. */
    private void drawTank(GuiGraphicsExtractor graphics, int x, int y) {
        ...
        int height = Math.round(waterBar.width(menu.waterStored(), menu.tankCapacity(), TANK_HEIGHT, menu.isSynced()));
        if (height > 0) {
            var model = Minecraft.getInstance().getModelManager().getFluidStateModelSet().get(Fluids.WATER.defaultFluidState());
            var sprite = model.stillMaterial().sprite();
            int color = model.fluidTintSource() == null ? 0xFFFFFFFF
                    : model.fluidTintSource().colorAsStack(new FluidStack(Fluids.WATER, FluidType.BUCKET_VOLUME));
    ...
    /** An empty bucket goes in at the top and comes back full at the bottom. */
    ...
    protected void extractTooltip(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        if (mouseX >= leftPos + TANK_X - 1 && mouseX < leftPos + TANK_X + TANK_WIDTH + 1
            ...
            graphics.setTooltipForNextFrame(Component.translatable("gui.futuretech.tank.contents",
                    Fluids.WATER.getFluidType().getDescription(), menu.waterStored(),
                    menu.tankCapacity()), mouseX, mouseY);
        }
    ...
    protected void extractLabels(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        // What it draws while pumping, and how many sources it is drawing from.
        Component rate = Component.translatable("gui.futuretech.rate", menu.isPumping() ? menu.energyRatePerTick() : 0);
        ...
        graphics.text(font, Component.translatable("gui.futuretech.tank.amount", menu.waterStored(),
                menu.tankCapacity()), 70, 73, TEXT, false);
        // What the pump makes a tick, on its own line under the tank's level.
        graphics.text(font, Component.translatable("gui.futuretech.water_pump.flow", menu.waterRatePerTick()), 70, 83, TEXT, false);
        Component sources = Component.translatable(menu.sources() < MIN_SOURCES
                ? "gui.futuretech.water_pump.sources_few" : "gui.futuretech.water_pump.sources", menu.sources());
        graphics.text(font, sources, 70, 36, menu.sources() < MIN_SOURCES ? 0xFFB03A2E : TEXT, false);
    }
}
