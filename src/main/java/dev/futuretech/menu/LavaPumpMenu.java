...
/** The lava pump's menu: the bucket slots, and the tank, status and pool the screen reads. */
public final class LavaPumpMenu extends MachineMenu implements SideConfigMenu, RedstoneControlMenu, EnergyInfoMenu {
    ...
    public LavaPumpMenu(int id, Inventory inventory) {
    ...
    private LavaPumpMenu(int id, Inventory inventory, ContainerData data) {
        this(id, inventory, new SimpleContainer(INVENTORY_SIZE),
                new UpgradeInventory(() -> Math.clamp(data.get(DATA_MK), 1, 4), () -> {}), data);
    }

    public LavaPumpMenu(int id, Inventory inventory, Container buckets, UpgradeInventory upgrades, ContainerData data) {
        super(ModMenus.LAVA_PUMP.get(), id);
        ...
        addSlot(new Slot(buckets, SLOT_INPUT, SLOT_X, INPUT_Y) {
            @Override
            public boolean mayPlace(ItemStack stack) { return LavaPumpBlockEntity.isFillableContainer(stack); }
        });
        ...
        addDataSlots(data);
        if (buckets instanceof LavaPumpBlockEntity) markSynced();
    }
    ...
    public int tankCapacity() { return LavaPumpBlockEntity.tankCapacity(mk()); }

    public LavaPumpBlockEntity.Status status() { return LavaPumpBlockEntity.Status.byOrdinal(data.get(DATA_STATUS)); }

    public boolean isPumping() { return status() == LavaPumpBlockEntity.Status.PUMPING; }

    /** Sources the last search found and the pump has not taken yet. */
    public int poolSize() { return data.get(DATA_POOL); }

    /** What the pump averages a tick while it has lava, at its level. */
    public int lavaRatePerTick() { return LavaPumpBlockEntity.ratePerTick(mk()); }
    ...
    @Override
    public BlockState displayState() { return ModBlocks.LAVA_PUMP.get().displayState(front()).setValue(MachineLevel.MK, mk()); }

    @Override
    public Set<SideMode> allowedModes() {
        return ((SideConfigurableBlock) ModBlocks.LAVA_PUMP.get()).allowedSideModes();
    }

    @Override
    public boolean supportsAutoPull() { return ModBlocks.LAVA_PUMP.get().supportsAutoPull(); }

    @Override
    public boolean supportsAutoPush() { return ModBlocks.LAVA_PUMP.get().supportsAutoPush(); }
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        } else if (LavaPumpBlockEntity.isFillableContainer(stack)) {
