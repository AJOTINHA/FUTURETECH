...
/**
 * The furnace's menu lays out one row of slots per open lane, centred on the panel, so the MK
...
public final class ElectricFurnaceMenu extends MachineMenu implements SideConfigMenu, RedstoneControlMenu, EnergyInfoMenu {
    ...
    private ElectricFurnaceMenu(int id, Inventory inventory, int mk) {
    ...
    private ElectricFurnaceMenu(int id, Inventory inventory, Container contents, UpgradeInventory upgrades, ContainerData data, int mk) {
    ...
    public static void writeOpeningData(RegistryFriendlyByteBuf buffer, ElectricFurnaceBlockEntity furnace) {
        buffer.writeVarInt(furnace.lanes());
    }
    ...
    @Override
    public BlockState displayState() { return ModBlocks.ELECTRIC_FURNACE.get().displayState(front()).setValue(MachineLevel.MK, Math.clamp(data.get(DATA_MK), 1, 4)); }
