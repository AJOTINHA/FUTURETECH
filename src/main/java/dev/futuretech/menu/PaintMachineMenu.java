...
/**
 * The paint machine's menu lays out one row per open lane — the block, the plates, then the
 * panels — so the MK travels in the opening packet: the client has to know how many rows to make
 * before any data slot arrives. The block slot takes only what a facade may wear and the plate
 * slot only steel plates, so what the player drops in is what the machine can use.
 */
public final class PaintMachineMenu extends MachineMenu implements SideConfigMenu, RedstoneControlMenu, EnergyInfoMenu {
    ...
    /** Width of the painter screen; upgrade slots sit in the tab beside it. */
    public static final int IMAGE_WIDTH = 176;
    /** The block and the plates sit side by side, then the arrow, then the panels. */
    ...
    public PaintMachineMenu(int id, Inventory inventory, RegistryFriendlyByteBuf buffer) {
    ...
    private PaintMachineMenu(int id, Inventory inventory, int mk) {
        this(id, inventory, new SimpleContainer(INVENTORY_SIZE), new UpgradeInventory(() -> mk, () -> {}), new SimpleContainerData(DATA_COUNT), mk);
    }

    public PaintMachineMenu(int id, Inventory inventory, Container contents, UpgradeInventory upgrades, ContainerData data) {
        this(id, inventory, contents, upgrades, data, data.get(DATA_MK));
    }

    private PaintMachineMenu(int id, Inventory inventory, Container contents, UpgradeInventory upgrades, ContainerData data, int mk) {
        super(ModMenus.PAINT_MACHINE.get(), id);
        ...
        // Menu slots go blocks, plates, outputs, mirroring the block entity's order.
        for (int lane = 0; lane < lanes; lane++) {
            addSlot(new Slot(contents, SLOT_BLOCK + lane, INPUT_A_X, rowY(lane)) {
                @Override
                public boolean mayPlace(ItemStack stack) { return PaintMachineBlockEntity.facadeOf(stack) != null; }
            });
        }
        for (int lane = 0; lane < lanes; lane++) {
            addSlot(new Slot(contents, SLOT_PLATES + lane, INPUT_B_X, rowY(lane)) {
                @Override
                public boolean mayPlace(ItemStack stack) { return PaintMachineBlockEntity.isPlate(stack); }
            });
        }
        for (int lane = 0; lane < lanes; lane++) {
        ...
        addDataSlots(data);
        if (contents instanceof PaintMachineBlockEntity) markSynced();
    }

    /** What to write when opening: the MK, which fixes the number of rows. */
    public static void writeOpeningData(RegistryFriendlyByteBuf buffer, PaintMachineBlockEntity painter) {
        buffer.writeVarInt(painter.lanes());
    }

    /** Open lanes, each a block slot and a plate slot with an output slot. */
    ...
    @Override
    public BlockState displayState() { return ModBlocks.PAINT_MACHINE.get().displayState(front()).setValue(MachineLevel.MK, Math.clamp(data.get(DATA_MK), 1, 4)); }

    @Override
    public Set<SideMode> allowedModes() {
        return ((SideConfigurableBlock) ModBlocks.PAINT_MACHINE.get()).allowedSideModes();
    }

    @Override
    public boolean supportsAutoPull() { return ModBlocks.PAINT_MACHINE.get().supportsAutoPull(); }

    @Override
    public boolean supportsAutoPush() { return ModBlocks.PAINT_MACHINE.get().supportsAutoPush(); }
    ...
    @Override
    public SlotRole slotRole(int index) { return index < 2 * lanes ? SlotRole.INPUT : index < 3 * lanes ? SlotRole.OUTPUT : SlotRole.NONE; }
    ...
    /**
     * Shift-clicking sends plates to the plate slots and blocks to the block slots, topping up
     * slots already holding the item before taking the first free one of its kind.
     */
    private boolean moveToInputs(ItemStack stack) {
        int first;
        if (PaintMachineBlockEntity.isPlate(stack)) first = lanes;
        else if (PaintMachineBlockEntity.facadeOf(stack) != null) first = 0;
        else return false;
        boolean moved = false;
        for (int slotIndex = first; slotIndex < first + lanes && !stack.isEmpty(); slotIndex++) {
        ...
        for (int slotIndex = first; slotIndex < first + lanes && !stack.isEmpty(); slotIndex++) {
            if (!slots.get(slotIndex).hasItem()) moved |= moveItemStackTo(stack, slotIndex, slotIndex + 1, false);
        }
        return moved;
    }
}
