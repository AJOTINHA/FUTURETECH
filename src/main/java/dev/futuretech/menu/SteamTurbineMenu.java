...
import static dev.futuretech.block.entity.SteamTurbineBlockEntity.*;
...
public final class SteamTurbineMenu extends MachineMenu implements SideConfigMenu, RedstoneControlMenu, dev.futuretech.api.gui.EnergyInfoMenu {
    private final ContainerData data;
    private final @Nullable SteamTurbineBlockEntity generator;

    /** Width of the turbine screen. */
    public static final int IMAGE_WIDTH = 176;

    public SteamTurbineMenu(int id, Inventory inventory) {
        this(id, inventory, new SimpleContainerData(DATA_COUNT));
    }

    /** Client data is synchronized by the server menu. */
    private SteamTurbineMenu(int id, Inventory inventory, ContainerData data) {
        this(id, inventory, null, new UpgradeInventory(() -> Math.clamp(data.get(DATA_MK), 1, 4), () -> {}), data);
    }

    public SteamTurbineMenu(int id, Inventory inventory, @Nullable SteamTurbineBlockEntity generator,
                            UpgradeInventory upgrades, ContainerData data) {
        super(ModMenus.STEAM_TURBINE.get(), id);
    ...
    @Override public int energyRatePerTick() { return Math.max(1, data.get(DATA_MAX_RATE)); }
    ...
    public BlockState displayState() {
        return ModBlocks.STEAM_TURBINE.get().displayState(front()).setValue(MachineLevel.MK, mk());
    }

    @Override
    public Set<SideMode> allowedModes() { return ModBlocks.STEAM_TURBINE.get().allowedSideModes(); }
