...
/**
 * The metal_press's menu lays out one row of slots per open lane, centred on the panel, so the MK
...
public final class MetalPressMenu extends MachineMenu implements SideConfigMenu, RedstoneControlMenu, EnergyInfoMenu {
    ...
    /** Width of the metal_press screen; upgrade slots sit in the tab beside it. */
    public static final int IMAGE_WIDTH = 176;
    public static final int INPUT_X = 56;
    public static final int OUTPUT_X = 104;
    public static final int MOLD_X = 80;
    ...
    public MetalPressMenu(int id, Inventory inventory, RegistryFriendlyByteBuf buffer) {
    ...
    private MetalPressMenu(int id, Inventory inventory, int mk) {
        this(id, inventory, new SimpleContainer(INVENTORY_SIZE), new UpgradeInventory(() -> mk, () -> {}), new SimpleContainerData(DATA_COUNT), mk);
    }

    public MetalPressMenu(int id, Inventory inventory, Container contents, UpgradeInventory upgrades, ContainerData data) {
        this(id, inventory, contents, upgrades, data, data.get(DATA_MK));
    }

    private MetalPressMenu(int id, Inventory inventory, Container contents, UpgradeInventory upgrades, ContainerData data, int mk) {
        super(ModMenus.METAL_PRESS.get(), id);
        ...
        for (int lane = 0; lane < lanes; lane++) addSlot(new Slot(contents, SLOT_INPUT + lane, INPUT_X, rowY(lane)) {
            @Override
            public boolean mayPlace(ItemStack stack) { return !MetalPressBlockEntity.isMold(stack); }
        });
        ...
        addSlot(new Slot(contents, SLOT_MOLD, MOLD_X, moldY()) {
            @Override
            public boolean mayPlace(ItemStack stack) { return MetalPressBlockEntity.isMold(stack); }
            @Override
        ...
        addDataSlots(data);
        if (contents instanceof MetalPressBlockEntity) markSynced();
    }

    /** What to write when opening: the MK, which fixes the number of rows. */
    public static void writeOpeningData(RegistryFriendlyByteBuf buffer, MetalPressBlockEntity metal_press) {
        buffer.writeVarInt(metal_press.lanes());
    }
    ...
    public int moldY() { return rowY(lanes - 1) + 20; }

    public boolean hasMold() { return MetalPressBlockEntity.isMold(contents.getItem(SLOT_MOLD)); }
    ...
    @Override
    public BlockState displayState() { return ModBlocks.METAL_PRESS.get().displayState(front()).setValue(MachineLevel.MK, Math.clamp(data.get(DATA_MK), 1, 4)); }

    @Override
    public Set<SideMode> allowedModes() {
        return ((SideConfigurableBlock) ModBlocks.METAL_PRESS.get()).allowedSideModes();
    }

    @Override
    public boolean supportsAutoPull() { return ModBlocks.METAL_PRESS.get().supportsAutoPull(); }

    @Override
    public boolean supportsAutoPush() { return ModBlocks.METAL_PRESS.get().supportsAutoPush(); }
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        } else if (MetalPressBlockEntity.isMold(stack)) {
            if (!moveItemStackTo(stack, 2 * lanes, 2 * lanes + 1, false)) return ItemStack.EMPTY;
