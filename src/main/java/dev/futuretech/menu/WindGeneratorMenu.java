...
public final class WindGeneratorMenu extends MachineMenu implements SideConfigMenu, RedstoneControlMenu, dev.futuretech.api.gui.EnergyInfoMenu {
    private final ContainerData data;
    private final @Nullable WindGeneratorBlockEntity generator;

    /** Width of the solar generator screen. */
    public static final int IMAGE_WIDTH = 176;

    public WindGeneratorMenu(int id, Inventory inventory) {
        this(id, inventory, new SimpleContainerData(DATA_COUNT));
    }

    /** Client data is synchronized by the server menu. */
    private WindGeneratorMenu(int id, Inventory inventory, ContainerData data) {
        this(id, inventory, null, data);
    }

    public WindGeneratorMenu(int id, Inventory inventory, @Nullable WindGeneratorBlockEntity generator, ContainerData data) {
        super(ModMenus.WIND_GENERATOR.get(), id);
    ...
    public int windStrength() { return data.get(DATA_WIND); }
    ...
    public BlockState displayState() {
        return ModBlocks.WIND_GENERATOR.get().displayState(front()).setValue(MachineLevel.MK, mk());
    }

    @Override
    public Set<SideMode> allowedModes() { return ModBlocks.WIND_GENERATOR.get().allowedSideModes(); }
